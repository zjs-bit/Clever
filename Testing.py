...
def add_edges(Aamt: int, Bamt: int, capA: int, capB: int):
    edge_list = set(
        (
            ...
            (capA, Bamt),
            (min(capA, Aamt + Bamt), max(0, Bamt - (capA - Aamt))),
        )
    )
...
def RiddlerBobvsSusan():
    ...
    for i in range(nsims):
        ...
        running_total += (susan_score > bob_score) * 1 + (
            susan_score == bob_score
        ) * 0.5

    return running_total / nsims


def sockquant(nsims: int) -> float:

    socks = [4, 4, 5, 5, 6, 6]
    check_pair = lambda pair: 4 in pair and 6 in pair
    sum_6w4 = 0

    for i in range(nsims):
        random.shuffle(socks)
        # print(socks)
        if check_pair(socks[:2]) or check_pair(socks[2:4]) or check_pair(socks[4:]):
            sum_6w4 += 1

    return sum_6w4 / nsims


def main() -> None:
    res = sockquant(nsims=1000000)
    print(res)
