...
class BoardArea(ABC):
    ...
    @abstractmethod
    def score_dice(self, *, dice_value: int, placement: tuple = None) -> None:
...
class VectArea(BoardArea):
    ...
    def validate_move(self, action: DiceAction) -> bool:
        return self.last_filled < self.length - 1

    def score(self) -> float:
        return np.dot(self.score_vect, self.current_board)

    def get_bonus(self, *args) -> List[Bonus]:
        """Gets the bonus obtained from the last played dice"""
        bonuses = [self.bonus_vect[self.last_filled]]
        return bonuses
...
class PurpleArea(VectArea):
    ...
    def __init__(self) -> None:
        ...
        self.bonus_vect = self.bonus_vect = self.bonus_vect = [
    ...
    def validate_move(self, action: DiceAction) -> bool:
        if not super().validate_move(action):
            ...
            self.last_filled > -1
            and action.value < self.current_board[self.last_filled] % 6
        ):
            return False
        else:
            return True

    def score_dice(self, dice_value: int, **kwargs):
        self.last_filled += 1
...
class OrangeArea(VectArea):
    ...
    def __init__(self) -> None:
        ...
        self.score_vect = [1 for i in range(self.length)]
        self.bonus_vect = [
            Bonus(BonusType.NONE),
            Bonus(BonusType.NONE),
            Bonus(BonusType.REROLL),
    ...
    def state(self):
        return {"score": self.score(), "last_filled": self.last_filled}
    ...
    def score_dice(self, dice_value: int, **kwargs) -> None:
        self.last_filled += 1
        self.current_board[self.last_filled] = (
            dice_value * self.multipliers[self.last_filled]
        )
...
class GreenArea(VectArea):
    ...
    def validate_move(self, action: DiceAction) -> bool:
        return (
            super().validate_move(action)
            and self.lower_bds[self.last_filled + 1] <= action.value
        )

    def score_dice(self, **kwargs) -> None:
...
class GridArea(BoardArea):
    ...
    def validate_move(self, action: DiceAction) -> bool:
        return self.current_board[action.placement] == 0
...
class BlueArea(GridArea):
    ...
    def __init__(self) -> None:
        ...
        self.fox_row: int = 2
        self.row_bonuses = [
            Bonus(BonusType.IMMEDIATE, Color.ORANGE, 5),
            Bonus(BonusType.DECISION, Color.YELLOW),
            Bonus(BonusType.NONE),
        ]
        self.column_bonuses = [
    ...
    def score(self) -> int:
        sqs_filled = int(np.sum(self.current_board))
    ...
    def validate_move(self, action: DiceAction) -> bool:
        return super().validate_move(
            action
        ) and action.placement == self._get_placement(action.value)

    def score_dice(self, dice_value: int, **kwargs) -> None:
        self.current_board[self._get_placement(dice_value)] = 1
...
class YellowArea(GridArea):
    def __init__(self) -> None:
        ...
        self.board_values = np.array(
            [[3, 6, 5, 0], [2, 1, 0, 5], [1, 0, 2, 4], [0, 3, 4, 6]]
        )
    ...
    def validate_move(self, action: DiceAction) -> bool:
        return (
            super().validate_move(action)
            and action.value == self.board_values[action.placement]
        )

    def score_dice(self, placement: tuple, **kwargs) -> None:
    ...
    def get_bonus(self, action: DiceAction) -> List[Bonus]:
        return (
            super().get_row_bonus(action.placement[0]),
            self.get_diag_bonus(action.placement),
        )

    def get_diag_bonus(self, placement: Tuple[int, int]) -> Bonus:
